...
static QVector<theme_palette_option> build_theme_options() {
    return {
...
static theme_palette_id find_palette_id_for_color(const QColor& color) {
...
static theme_palette_id find_palette_id_for_label(const QString& label) {
    const auto& palettes = theme_palette_registry::options();
    for (int i = 0; i < palettes.size(); ++i) {
...
theme_palette_id theme_palette_registry::id_from_color(const QColor& color) {
    return find_palette_id_for_color(color);
}

theme_palette_id theme_palette_registry::id_from_label(const QString& label) {
    return find_palette_id_for_label(label);
}
