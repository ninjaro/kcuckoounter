...
static void insert_svg_result(
    raster_cache& cache_service, int bucket_px, int generation
) {
...
static bool wait_for_message_families(
    QLocalSocket* socket, QByteArray* received_jsonl,
    const QSet<QString>& required_families, int timeout_ms
) {
    if (socket == nullptr || received_jsonl == nullptr) {
        return false;
    }
...
void resource_monitor_tests::
    ...
    QVERIFY2(
        args.at(1).toBool(),
        qPrintable(
            QStringLiteral("%1 | %2").arg(output_path, args.at(2).toString())
        )
    );
...
void resource_monitor_tests::
    ...
    insert_svg_result(cache_service, 96, 1);
    QTest::qWait(50);
    QCOMPARE(process_export_spy.count(), 0);

    insert_svg_result(cache_service, 128, 2);
    QTest::qWait(50);
    QCOMPARE(process_export_spy.count(), 0);

    insert_svg_result(cache_service, 160, 3);
    QTRY_VERIFY(process_export_spy.count() >= 1);
...
void resource_monitor_tests::auto_export_is_rate_limited_by_mode_window() {
    ...
    for (int index = 0; index < 2; ++index) {
        insert_svg_result(cache_service, 100 + index * 16, index + 1);
    ...
    insert_svg_result(cache_service, 200, 10);
    QTest::qWait(120);
    insert_svg_result(cache_service, 220, 11);
    QTest::qWait(120);
